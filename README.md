...
- `Codigos Operarios.docx`: 192 operarios validos en `fleet-data.js` y `google-apps-script/Data.gs`.
...
La web abre directamente ese vehiculo. El operario escribe su codigo de 3 digitos o su nombre, y la app autocompleta el otro campo.
...
- `Operarios`: columnas `codigo`, `nombre`, `apellidos`, `nombre_completo`.
- `Registros`: movimientos de recogida/devolucion.

Si `Furgonetas` u `Operarios` estan vacias, `setup` las rellena con los datos extraidos de tus archivos.
...
2. Escribe un codigo de operario, por ejemplo `188`.
3. Pulsa `En curso` y `Guardar registro`.
4. Comprueba que aparece una fila `En curso` en `Registros`.
5. Vuelve a abrir el mismo QR: debe seguir saliendo `En curso`.
6. Pulsa `Devolver` y guarda: la misma fila debe pasar a `Devuelto`.
