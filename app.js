(function () {
  ...
  function init() {
    els.companyName.textContent = config.companyName || "Mi empresa";
    renderVanOptions();
  ...
  function restoreDriver() {
    const savedCode = localStorage.getItem("fleetDriverCode");
    const savedName = localStorage.getItem("fleetDriverName");
    if (savedCode) {
      els.driverCode.value = savedCode;
    }
    if (savedName) {
      els.driverName.value = savedName;
    }
  }
  ...
  function handleNameInput() {
    ...
    if (value.length >= 4) {
      ...
      if (matches.length === 1) {
        ...
        localStorage.setItem("fleetDriverCode", matches[0].code);
      }
    }
  }
  ...
  function renderStatus(response) {
    ...
    if (isOpen && response.openRecord) {
      ...
      maybePrefillFromOpenRecord(open);
      setAction("return");
    } else {
      els.openMovement.textContent = "Ninguno";
      setAction("pickup");
  ...
  function maybePrefillFromOpenRecord(open) {
    if (!els.driverCode.value && open.operatorCode) {
      els.driverCode.value = open.operatorCode;
      localStorage.setItem("fleetDriverCode", open.operatorCode);
    }

    if (!els.driverName.value && open.operatorFullName) {
      els.driverName.value = open.operatorFullName;
      localStorage.setItem("fleetDriverName", open.operatorFullName);
    }
  }
