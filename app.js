(function () {
  ...
  function renderOperatorOptions() {
    ...
    operators.forEach((operator) => {
      ...
      nameOption.label = `Codigo ${operator.code}`;
  ...
  async function handleSubmit(event) {
    ...
    if (!els.driverCode.value.trim()) {
      setMessage("error", "El codigo de operario es obligatorio.");
    ...
    if (!operator) {
      setMessage("error", "Selecciona un operario valido y revisa que codigo y nombre coincidan.");
      els.driverName.focus();
      return;
    }

    if (canonicalCode(els.driverCode.value) !== operator.code) {
      setMessage("error", "El codigo de operario debe coincidir con el nombre seleccionado.");
      els.driverCode.focus();
      return;
    }
