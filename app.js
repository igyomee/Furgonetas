(function () {
  ...
  function renderOperatorOptions() {
    ...
    operators.forEach((operator) => {
      ...
      nameOption.label = operator.code ? `Codigo ${operator.code}` : "Sin codigo";
  ...
  async function handleSubmit(event) {
    ...
    const operator = resolveOperatorForSubmit();
    if (!operator) {
      setMessage("error", "Selecciona un operario valido de la lista.");
      els.driverName.focus();
      return;
    }

    if (operator.code && canonicalCode(els.driverCode.value) !== operator.code) {
      setMessage("error", "Este operario tiene codigo. El codigo es obligatorio y debe coincidir.");
      els.driverCode.focus();
      return;
    }

    if (!operator.code && els.driverCode.value.trim()) {
      setMessage("error", "Este operario no tiene codigo asignado. Deja el codigo vacio.");
      els.driverCode.focus();
      return;
    }

    if (normalizeDni(els.driverDni.value) !== normalizeDni(operator.dni)) {
      setMessage("error", "El DNI no corresponde con el operario seleccionado.");
      els.driverDni.focus();
      return;
    }
  ...
  function resolveOperatorForSubmit() {
    const code = canonicalCode(els.driverCode.value);
    const byName = findOperatorByName(els.driverName.value);

    if (code) {
      const byCode = findOperatorByCode(code);
      if (!byCode) {
        return null;
      }
      return byName && byName !== byCode ? null : byCode;
    }
